...
with app.app_context():
    ...
    positions_data = [
        ...
        ("Staff Senior", 7000000, "Staff dengan pengalaman > 3 tahun"),
    ...
    employees_data = [
        ...
        # 20 new employees (EMP011 - EMP030)
        {
            "username": "emp011",
            "code": "EMP011",
        ...
        },
        {
            "username": "emp016",
            "code": "EMP016",
            "name": "Pramono Adi Wibowo",
            "email": "pramono@example.com",
            "phone": "08272233445",
            "address": "Bogor",
            "position": "Supervisor",
    ...
    for ed in employees_data:
        ...
        if not user:
            user = User(username=ed["username"], role="employee")
        ...
        if not emp:
            ...
            # Link user_id jika belum ter-link
            if not emp.user_id:
                emp.user_id = user.id
    ...
    for name, ctype, amount, desc in components_data:
        if not SalaryComponent.query.filter_by(name=name).first():
            sc = SalaryComponent(
                ...
                default_amount=amount, description=desc
            )
            db.session.add(sc)
            db.session.flush()
    ...
    # ===================== ATTENDANCES =====================
    # Ambil semua employee untuk absensi
    all_emps = Employee.query.order_by(Employee.id).all()

    attendance_data = [
        (all_emps[0], "2026-07", 22, 0, 5),   # EMP001
        (all_emps[1], "2026-07", 22, 1, 3),   # EMP002
        (all_emps[2], "2026-07", 22, 2, 4),   # EMP003
        (all_emps[3], "2026-07", 22, 0, 2),   # EMP004
        (all_emps[4], "2026-07", 22, 1, 0),   # EMP005
        (all_emps[5], "2026-07", 22, 0, 3),   # EMP006
        (all_emps[6], "2026-07", 22, 1, 1),   # EMP007
        (all_emps[7], "2026-07", 22, 3, 0),   # EMP008
        (all_emps[8], "2026-07", 22, 0, 0),   # EMP009
        (all_emps[9], "2026-07", 22, 2, 2),   # EMP010
        (all_emps[10], "2026-07", 22, 0, 4),  # EMP011
        (all_emps[11], "2026-07", 22, 1, 2),  # EMP012
        (all_emps[12], "2026-07", 22, 0, 0),  # EMP013
        (all_emps[13], "2026-07", 22, 2, 3),  # EMP014
        (all_emps[14], "2026-07", 22, 0, 1),  # EMP015
    ]
    for emp, period, work, absent, overtime in attendance_data:
        if emp and not Attendance.query.filter_by(employee_id=emp.id, period=period).first():
            att = Attendance(
                ...
                work_days=work, absent_days=absent,
